...
| `since`   | `String` | Information since when a model element is deprecated e.g. a concrete version or date. |
| `removedWith`   | `String` | Information when the deprecated element will no longer be available e.g. a version or date. |
| `removalDate`   | `String` | Explicit date when the deprecation will be removed from the code base.<br><br>The expected date format is `yyyy-mm-dd`. |
